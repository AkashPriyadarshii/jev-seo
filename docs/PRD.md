...
- **Shipped as v0.1.2 on 26 September 2026**: rule engine R01-R58, unified rules scoring, CSV export, rescore, RunManifest + citation gates, Jev spend budget, action tracker CSV, `explain`, `report --baseline`, narrative.json, pair cannibalization A×B, 15-tool MCP (`seo_cite_check`, `seo_gap`, keyless GEO, drift alerts), skill-hard Jev thresholds, designed PDF deck, PageSpeed vitals, opt-in DataForSEO, eval second-judge protocol.
